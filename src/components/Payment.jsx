...
const EVM_WALLET_ADDRESS = '0x1483393Cabbc486fe15B2fD067Bf91A41fc17a1a';
const TRON_WALLET_ADDRESS = 'TKYwnabAbfg9Tv7TVV7BCMk6indjuzeV32';
...
function Payment() {
  ...
  const walletAddress = network === 'TRC-20' ? TRON_WALLET_ADDRESS : EVM_WALLET_ADDRESS;
  const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=220x220&margin=12&data=${encodeURIComponent(walletAddress)}`;
  const explorerBase = network === 'BEP-20' ? 'https://bscscan.com/tx/' : network === 'ERC-20' ? 'https://etherscan.io/tx/' : 'https://tronscan.org/#/transaction/';

  const copyWallet = async () => { await navigator.clipboard.writeText(walletAddress); setCopied(true); setTimeout(() => setCopied(false), 1600); };
  const extractTxid = (value) => {
    const text = String(value || '').trim();
    if (network === 'TRC-20') { const match = text.match(/(?:^|\/)([a-fA-F0-9]{64})(?:$|[?#/])/); return match ? match[1] : (/^[a-fA-F0-9]{64}$/.test(text) ? text : ''); }
    const match = text.match(/0x[a-fA-F0-9]{64}/); return match ? match[0] : '';
  };

  const emailReceipt = async ({ txid, details }) => {
    setEmailStatus('sending');
    try {
      const response = await fetch('/api/send-receipt', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: form.name, email: form.email, project: form.project, service: form.service, amount: details?.amount || form.amount, network, invoiceId, txid, explorerUrl: `${explorerBase}${txid}` }) });
  ...
  const submitPayment = async (e) => {
    ...
    if (!txid) { setVerification({ status: 'failed', message: `Paste a valid TxID or ${network === 'TRC-20' ? 'Tronscan' : network === 'BEP-20' ? 'BscScan' : 'Etherscan'} transaction URL.`, details: null }); return; }
  ...
  const txPlaceholder = network === 'BEP-20' ? 'TxID or BscScan link' : network === 'ERC-20' ? 'TxID or Etherscan link' : 'TxID or Tronscan link';
  ...
  return <section className="payment-section" id="payment"><div className="payment-shell" data-aos="fade-up">
    ...
    <div className="payment-layout">
      <div className="payment-card payment-details-card">
        ...
        <label className="payment-label">Select network</label><div className="network-tabs">{['BEP-20', 'ERC-20', 'TRC-20'].map(item => <button type="button" key={item} className={network === item ? 'network-tab active' : 'network-tab'} onClick={() => { setNetwork(item); setVerification({ status: 'idle', message: '', details: null }); setCopied(false); setShowQr(false); }}>{item}</button>)}</div>
        <div className="wallet-box"><div><span className="payment-label">USDT {network} wallet</span><p>{walletAddress}</p></div><button type="button" className="copy-wallet-btn" onClick={copyWallet}>{copied ? 'Copied ✓' : 'Copy'}</button></div>
