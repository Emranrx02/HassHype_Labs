...
function Payment() {
  ...
  const submitPayment = (e) => { e.preventDefault(); setSubmitted(true); };
      ...
      <div className="payment-shell" data-aos="fade-up">
        ...
        <div className="payment-layout">
          <div className="payment-card payment-details-card">
            <div className="payment-card-top"><div><span className="payment-label">PAY WITH</span><h3>USDT</h3></div><span className="payment-status-dot">Crypto Payment</span></div>
            <label className="payment-label">Select network</label>
            <div className="network-tabs">
              {['BEP-20', 'ERC-20'].map((item) => <button type="button" key={item} className={network === item ? 'network-tab active' : 'network-tab'} onClick={() => setNetwork(item)}>{item}</button>)}
            </div>
            ...
            <div className="payment-warning"><span>!</span><p>Only send USDT using the selected network. Sending another token or using a different network may result in loss of funds.</p></div>
            <div className="payment-company">
              <div><span>Merchant</span><strong>HashHype Labs</strong></div><div><span>Website</span><strong>hashhypelabs.com</strong></div><div><span>Support</span><strong>@emranrx</strong></div><div><span>Receipt</span><strong>USD / USDT</strong></div>
            </div>
          </div>
          ...
          <form className="payment-card payment-form" onSubmit={submitPayment}>
            ...
            <div className="payment-field-grid">
              <label>Client name<input name="name" value={form.name} onChange={update} placeholder="Your full name" required /></label>
              <label>Email address<input name="email" type="email" value={form.email} onChange={update} placeholder="you@company.com" required /></label>
            </div>
            ...
            <div className="payment-field-grid">
              <label>Amount<div className="amount-input"><input name="amount" type="number" min="1" step="0.01" value={form.amount} onChange={update} placeholder="500" required /><span>USDT</span></div></label>
              <label>Network<input value={network} readOnly /></label>
            </div>
            <label>Transaction Hash / TxID<input name="txid" value={form.txid} onChange={update} placeholder="Paste transaction hash after payment" required /></label>
        ...
        {submitted && <div className="invoice-preview" data-aos="zoom-in">
          <div className="invoice-success">✓ PAYMENT SUBMITTED</div>
          <div className="invoice-header"><div><span>HASHHYPE LABS</span><h3>Payment Receipt</h3></div><div className="invoice-pending">PENDING VERIFICATION</div></div>
          <div className="invoice-grid"><div><span>Invoice ID</span><strong>{invoiceId}</strong></div><div><span>Client</span><strong>{form.name}</strong></div><div><span>Email</span><strong>{form.email}</strong></div><div><span>Project</span><strong>{form.project}</strong></div><div><span>Service</span><strong>{form.service}</strong></div><div><span>Amount</span><strong>{form.amount} USDT</strong></div><div><span>Network</span><strong>{network}</strong></div><div><span>Payment method</span><strong>USDT Crypto</strong></div></div>
